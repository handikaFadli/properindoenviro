                    ...
                    <table class="w-full text-sm text-left">
                        ...
                        @foreach($roles as $i => $role)
                            <tr class="border-b border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                <td class="px-4 py-3 text-xs text-gray-400">{{ $roles->firstItem() + $i }}</td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-center gap-1">
                                        <button type="button"
                                                onclick="openEditModal({{ $role->id }}, @js($role->name))"
                                        ...
                                        <form id="delete-form-{{ $role->id }}" action="{{ route('roles.destroy', $role->id) }}" method="POST" class="inline">
                                            ...
                                            @method('DELETE')
                                            <button type="button"
                                                    onclick="openDeleteModal('delete-form-{{ $role->id }}', '{{ $role->name }}')"
