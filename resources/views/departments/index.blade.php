...
<main>
    ...
    <div class="pt-5 flex items-center justify-between mb-5">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Daftar Departemen</h1>
        <button type="button"
                    ...
                    <table class="w-full text-sm text-left">
                        ...
                        @foreach($departments as $i => $department)
                            <tr class="border-b border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                <td class="px-4 py-3 text-xs text-gray-400">{{ $departments->firstItem() + $i }}</td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-center gap-1">
                                        <button type="button"
                                                onclick="openEditModal({{ $department->id }},@js($department->name))"
                                        ...
                                        <form id="delete-form-{{ $department->id }}" action="{{ route('departments.destroy', $department->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                                    onclick="openDeleteModal('delete-form-{{ $department->id }}', '{{ $department->name }}')"
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
        ...
        <div class="text-sm text-gray-500">
            Menampilkan
            <span class="font-semibold">{{ $departments->firstItem() ?? 0 }}</span>
            -
            <span class="font-semibold">{{ $departments->lastItem() ?? 0 }}</span>
            dari
            <span class="font-semibold">{{ $departments->total() }}</span>
            data
        </div>
